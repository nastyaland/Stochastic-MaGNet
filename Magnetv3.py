...
class MaGNet(nn.Module):
    def __init__(self, N, T, F, dim, num_MAGE, num_experts,
        ...
        self.mc_dropout_mage = nn.Dropout(dropout) # the MC dropout after the MAGE block
        self.mc_dropout_feature = nn.Dropout(dropout) # the MC dropout after the feature attention block
        self.mc_dropout_final = nn.Dropout(dropout) # the MC dropout before the final prediction
    ...
    def forward(self, x):
        ...
        x = self.mc_dropout_mage(x)

        x, att_scores_F = self.SelfAttention2D_F(x)
        
        x = self.mc_dropout_feature(x)
